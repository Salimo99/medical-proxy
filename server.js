...
const fs = require('fs'); // حزمة مدمجة لقراءة الملفات
...
// قراءة مفتاح Gemini من متغيرات البيئة بآمان
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;

app.post('/ask-bot', async (req, res) => {
    try {
        const { userQuery } = req.body; // استلام سؤال المستخدم فقط

        if (!userQuery) {
            return res.status(400).json({ error: "سؤال المستخدم مطلوب" });
        }

        // 1. قراءة ملف الأدوية المستضاف على السيرفر محلياً
        ...
        // 2. رابط Gemini 2.5 Flash الرسمي
